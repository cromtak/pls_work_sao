...
package net.mcreator.sao.keybind;
...
public class OpenMenuKeyBinding extends SaoModElements.ModElement {
	@OnlyIn(Dist.CLIENT)
	private KeyBinding keys;

	public OpenMenuKeyBinding(SaoModElements instance) {
		super(instance, 12);
	...
	public void initElements() {
		keys = new KeyBinding("key.sao.open_menu", GLFW.GLFW_KEY_TAB, "key.categories.inventory");
		...
		if (Minecraft.getInstance().currentScreen == null) {
			if (event.getKey() == keys.getKey().getKeyCode()) {
				if (event.getAction() == GLFW.GLFW_PRESS) {
					SaoMod.PACKET_HANDLER.sendToServer(new KeyBindingPressedMessage(0, 0));
